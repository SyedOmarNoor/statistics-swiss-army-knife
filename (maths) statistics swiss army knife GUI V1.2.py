...
class Application(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.labeldict = {  # function name, label, frame
            ...
            'min_out':  ['Minor outlier',           "Outliers"], 
            'maj_out':  ['Major outlier',           "Outliers"], 
            'summary':  ['5-point summary',         "Summary"], 
        }

        print(self.labeldict)

          # tickboxes, not implemented yet so don't display for now
        ...
        for label in range(1, len(self.checkboxlabels)+1):
            exec(f"self.varcheck{label} = tk.IntVar(value=1)\ntk.Checkbutton(tickboxframe, text='{self.checkboxlabels[label-1]}', variable=self.varcheck{label}, command=self.checkboxupdate).grid(row=0, column = {label}, sticky='w')")
        
        tickboxframe.grid(row=1, columnspan = 2, column = 0, sticky = 'w')
        

        """  #
        for labels in self.labelvarlist:  # labels for values
            exec(
                f"self.{labels}label = tk.Label(self, text = '{self.labellist[self.labelvarlist.index(labels)]}: ').grid(row = {self.labelvarlist.index(labels)+2}, column = 0, sticky = 'e')"
                )

        for labels in self.labelvarlist:  # values
    ...
    def calculate(self, event=0):
        ...
        except ZeroDivisionError:  # except no values i.e. list length is 0
            return
root = tk.Tk()
