...
class Application(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        # list all fields, their label and their category
...
tk.Checkbutton(
    tickboxframe,
    text='{self.checkboxlabels[label-1]}',
    variable=self.varcheck{label},
    command=self.checkboxupdate
    ).grid(row=0, column = {label}, sticky='w')
"""
                # can't indent multi-line strings when exec
                # because it's say "unexpected indent"
            )
...
self.{func}label.grid(
row = {self.labelvarlist.index(func)+2},
column = 0,
sticky = 'e')
"""
            )

        for func, info in self.labeldict.items():  # values of list statistics
            exec(
                f"""
self.{func}result = tk.Label(self, text = '0')
self.{func}result.grid(
    row={self.labelvarlist.index(func) + 2}, column = 1, sticky = 'w'
)
"""
            )
        self.entrylabel = tk.Label(
            self,
            text="Enter list of numbers (separate with commas or spaces):"
        ).grid(row=0, column=0)

        # a decent width, doesn't adjust itself tho
        self.listentry = tk.Entry(self, width=46)
        self.listentry.grid(row=0, column=1, sticky='w')

        # calculate when key pressed
        self.listentry.bind('<Key>', self.calculate)
        # calculate when key released, just in case
        self.listentry.bind('<KeyRelease>', self.calculate)

    def checkboxupdate(self):
        for index, label in enumerate(self.checkboxlabels):
            index += 1
            if eval(f"self.varcheck{index}.get()"):  # if checkbox is true
